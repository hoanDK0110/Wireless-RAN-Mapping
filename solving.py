...
def short_term(num_slices, num_UEs, num_RUs, num_RBs, rb_bandwidth, P_i, gain, R_min, arr_pi_sk, arr_phi_i_sk, epsilon, logger=None):
    try:
...
def long_term(num_slices, num_UEs, num_RUs, num_DUs, num_CUs, num_RBs, P_i, rb_bandwidth, D_j, D_m, R_min, gain, A_j, A_m, l_ru_du, l_du_cu, epsilon, gamma, logger=None):
    try:
        ...
        objective = cp.Maximize(gamma * cp.sum(pi_sk) + (1 - gamma) * cp.sum([mu_ib_sk[i, b, s, k] for i in range(num_RUs) for b in range(num_RBs) for s in range(num_slices) for k in range(num_UEs)]) * 1e-4)
        objective = cp.Maximize(gamma * cp.sum(pi_sk) + (1 - gamma) * cp.sum([mu_ib_sk[i, b, s, k] for i in range(num_RUs) for b in range(num_RBs) for s in range(num_slices) for k in range(num_UEs)]) * 1e-4)
