...
def main():
    ...
    for gamma in [0, 0.2, 0.4, 0.5, 0.6, 0.8, 1]:
        ...
        for step in range(num_step):
            ...
            random_pi_sk, random_z_ib_sk, random_phi_i_sk, random_phi_j_sk, random_phi_m_sk, random_total_R_sk, random_total_pi_sk, random_objective = solving.mapping_RU_random_UE(num_slices, num_UEs, num_RUs, num_DUs, num_CUs, num_RBs, D_j, D_m, R_min, A_j, A_m, l_ru_du, l_du_cu, epsilon, gamma, slice_mapping, random_phi_i_sk, data_rate, P_ib_sk, max_tx_power_mwatts)
            random_end_time = time.time()
            random_time = random_end_time - random_start_time
            # Lưu kết quả long_term
            other_function.save_results("global", random_time, random_total_pi_sk, random_objective, output_folder_gamma)
            print(f"Long-term algorithm completed in {random_time:.4f} seconds.")
